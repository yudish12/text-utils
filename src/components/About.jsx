...
function About(){
  let [txt,setTxt] = React.useState("Enable Dark Mode");
    let [style,setStyle] = React.useState({});
    function DarkMode(){
        if(style.color==='white'){
          setTxt("Enable Dark Mode");
            setStyle({transition:"all 0.5s ease-out"});
        }
        else{
    ...
    return (
        <div style={style}>
            ...
            <h1 className='display-3 mx-2 my-5'>About Us</h1>
  ...
  <button className='btn btn-primary my-4' onClick={DarkMode}>{txt}</button>
</div>
        </div>
    )
}
